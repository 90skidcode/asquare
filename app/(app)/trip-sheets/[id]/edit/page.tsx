...
export default async function EditTripSheetPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const [trip, customers, vehicles, dealers, investors] = await Promise.all([
    prisma.tripSheet.findUnique({ where: { id } }),
  ...
  return (
    <div className="space-y-5">
      <PageHeader title={`Edit ${trip.tripNumber}`} description="Update trip sheet details." />
      <Card>
